...
# Version:      1.00                                                #
#                                                                   #
# Changes:      First version for wikidotjs (1.00)                    #
#                                                                   #
#                                                                   #
#####################################################################

configure_locale() {
  /usr/bin/logger 'configure_locale()' -t 'wikidotjs-bootstrap';
...
__EOF__
  ...
  /usr/bin/logger 'configure_locale() finished' -t 'wikidotjs-bootstrap';
}

configure_timezone() {
  /usr/bin/logger 'configure_timezone()' -t 'wikidotjs-bootstrap';
  ...
  /usr/bin/logger 'configure_timezone() finished' -t 'wikidotjs-bootstrap';
}

bootstrap_prerequisites() {
  # Install prerequisites and useful tools
  /usr/bin/logger 'bootstrap_prerequisites()' -t 'wikidotjs-bootstrap';
  ...
  /usr/bin/logger 'bootstrap_prerequisites() finished' -t 'wikidotjs-bootstrap';
}
...
public_ssh_keys() {
  ...
  /usr/bin/logger 'public_ssh_keys()' -t 'wikidotjs-bootstrap';
}
...
main() {
  ...
  /usr/bin/logger '!!!!! Main routine starting' -t 'wikidotjs-bootstrap';
  ...
  if [ "$HOSTNAME" = "wiki" ];
  then
    echo -e "\e[1;32m - Installing Wiki.js Wiki and CMS Server $HOSTNAME\e[0m";
    /root/install-wiki.sh;
  fi
  echo -e "\e[32m - bootstrap main() finished\e[0m";
  /usr/bin/logger 'installation finished (Main routine finished)' -t 'wikidotjs-bootstrap'; 
}
